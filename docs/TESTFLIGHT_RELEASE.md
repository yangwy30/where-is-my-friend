...
- APNs：production
- Archive（本机临时生成，不提交 Git）：`/tmp/AcrossUs-Staging-TestFlight-build10.xcarchive`
...
xcodebuild -exportArchive \
  -archivePath '/tmp/AcrossUs-Staging-TestFlight-build10.xcarchive' \
  -exportPath '/tmp/AcrossUs-Staging-TestFlight-build10-upload' \
...
3. 把构建 `0.1.0 (10)` 加进外部组，粘贴上面的 What to Test。
