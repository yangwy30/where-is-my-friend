...
- APNs：production
- Archive（本机临时生成，不提交 Git）：`/tmp/AcrossUs-Staging-TestFlight-build7.xcarchive`
...
xcodebuild -exportArchive \
  -archivePath '/tmp/AcrossUs-Staging-TestFlight-build7.xcarchive' \
  -exportPath '/tmp/AcrossUs-Staging-TestFlight-build7-upload' \
...
3. 把构建 `0.1.0 (7)` 加进外部组，粘贴上面的 What to Test。
