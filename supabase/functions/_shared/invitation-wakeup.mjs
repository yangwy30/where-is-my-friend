...
export function wakeInvitationWorker({baseURL, secret, waitUntil, fetcher=fetch, action="invitations"}) {
    if(!["invitations","trip-reminders"].includes(action)) return false;
    ...
    const task=Promise.resolve().then(()=>fetcher(url,{method:'POST',headers:{Authorization:`Bearer ${secret}`,'Content-Type':'application/json'},
        body:JSON.stringify({action}),signal:AbortSignal.timeout(8000)})
    )
